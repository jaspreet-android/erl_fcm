...
-type pool_type() :: rediscluster | redis | http | rdbms | cassandra | elastic | generic
                     | rabbit | ldap.
...
-type pool_record_result() :: {ok, #fcm_wpool{}} | {error, pool_error()}.
...
ensure_started() ->
    wpool:start(),
    case whereis(fcm_wpool_sup) of
        undefined ->
            fcm_wpool_sup:start_link();
        _ ->
            ok
    end,
    fcm_rdbms:create_ets_table(
      ...
      [named_table, public,
       ...
       {keypos, #fcm_wpool.name},
       {heir, whereis(fcm_wpool_sup), undefined}]).

start_configured_pools() ->
    Pools = application:get_env(erl_fcm, outgoing_pools, undefined),
...
-spec start(pool_type(), host_type_or_global(), tag(),
    ...
    start_result().
start(PoolType, HostType, Tag, WpoolOptsIn, ConnOpts, Strategy, CallTimeout) ->
  io:format("fcm_wpool start"),
    case fcm_wpool_mgr:start(PoolType, HostType, Tag, WpoolOptsIn, ConnOpts) of
        {ok, Pid} ->
            ets:insert(?MODULE, #fcm_wpool{name = {PoolType, HostType, Tag},
...
start_sup_pool(PoolType, ProcName, WpoolOpts) ->
    SupName = fcm_wpool_type_sup:name(PoolType),
...
stop(PoolType, HostType, Tag) ->
    try
        ...
        fcm_wpool_mgr:stop(PoolType, HostType, Tag)
    catch
        C:R:S ->
...
get_worker(PoolType, HostType, Tag) ->
    case get_pool(PoolType, HostType, Tag) of
        {ok, #fcm_wpool{strategy = Strategy} = Pool} ->
...
call(PoolType, HostType, Tag, Request) ->
    case get_pool(PoolType, HostType, Tag) of
        {ok, #fcm_wpool{strategy = Strategy, call_timeout = CallTimeout} = Pool} ->
...
call(PoolType, HostType, Tag, HashKey, Request) ->
    case get_pool(PoolType, HostType, Tag) of
        {ok, #fcm_wpool{call_timeout = CallTimeout} = Pool} ->
...
get_pools() ->
    lists:map(fun(#fcm_wpool{name = Name}) -> Name end, ets:tab2list(?MODULE)).
...
make_pool_name(PoolType, HostType, Tag) when is_binary(HostType) ->
    binary_to_atom(<<"fcm_wpool$", (atom_to_binary(PoolType, utf8))/binary, $$,
                     HostType/binary, $$, (atom_to_binary(Tag, utf8))/binary>>, utf8).

make_pool_name(#fcm_wpool{atom_name = undefined, name = {PoolType, HostType, Tag}}) ->
    make_pool_name(PoolType, HostType, Tag);
make_pool_name(#fcm_wpool{atom_name = AtomName}) ->
    AtomName.
...
make_callback_module_name(PoolType) ->
    Name = "fcm_wpool_" ++ atom_to_list(PoolType),
    list_to_atom(Name).
