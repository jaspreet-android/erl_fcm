...
-define(STATE_KEY, fcm_rdbms_state).
...
do_transaction_with_delayed_retry(HostType, F, Retries, Delay, Info) ->
    Result = fcm_rdbms:sql_transaction(HostType, F),
    case Result of
        {atomic, _} ->
            Result;
...
sql_call0(HostType, Msg) ->
    ...
    fcm_wpool:call(rdbms, HostType, {sql_cmd, Msg, Timestamp}).
...
sql_request0(HostType, Msg) ->
    Timestamp = erlang:monotonic_time(millisecond),
    fcm_wpool:send_request(rdbms, HostType, {sql_cmd, Msg, Timestamp}).
...
sql_cast0(HostType, Msg) ->
    Timestamp = erlang:monotonic_time(millisecond),
    fcm_wpool:cast(rdbms, HostType, {sql_cmd, Msg, Timestamp}).
...
get_db_info(HostType) ->
    fcm_wpool:call(rdbms, HostType, get_db_info).
...
escape_binary(_HostType, Bin) when is_binary(Bin) ->
    {escaped_binary, fcm_rdbms_backend:escape_binary(Bin)}.
...
escape_string_internal(S) ->
    case fcm_backend:is_exported(global, ?MODULE, escape_string, 1) of
        true ->
            fcm_rdbms_backend:escape_string(S);
        false ->
            %% generic escaping
            [$', escape_characters(S), $']
    end.
...
unescape_binary(_HostType, Bin) when is_binary(Bin) ->
    fcm_rdbms_backend:unescape_binary(Bin).
...
terminate(_Reason, #state{db_ref = DbRef}) ->
    catch fcm_rdbms_backend:disconnect(DbRef).
...
sql_query_internal(Query, #state{db_ref = DBRef, query_timeout = QueryTimeout}) ->
    case fcm_rdbms_backend:query(DBRef, Query, QueryTimeout) of
...
sql_execute(Type, Name, Params, State = #state{db_ref = DBRef, query_timeout = QueryTimeout}) ->
    ...
    put_state(NewState),
    Res = try fcm_rdbms_backend:execute(DBRef, StatementRef, Params, QueryTimeout)
...
prepare_statement(Name, State = #state{db_ref = DBRef, prepared = Prepared}) ->
    case maps:get(Name, Prepared, undefined) of
        undefined ->
            ...
            {ok, Ref} = fcm_rdbms_backend:prepare(DBRef, Name, Table, Fields, Statement),
...
db_engine(_HostType) ->
    try fcm_backend:get_backend_name(global, ?MODULE)
...
connect(#{query_timeout := QueryTimeout} = Options, Retry, RetryAfter, MaxRetryDelay) ->
    case fcm_rdbms_backend:connect(Options, QueryTimeout) of
        {ok, _} = Ok ->
            Ok;
        Error when Retry =:= 0 ->
            Error;
        Error ->
            ...
            Backend = fcm_backend:get_backend_name(global, ?MODULE),
